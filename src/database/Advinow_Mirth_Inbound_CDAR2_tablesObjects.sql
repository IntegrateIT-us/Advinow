/*	
	Filename: Advinow_Mirth_Inbound_CDAR2_tableObjects,sql
	Owner:	  Advinow
	Authored: Jeffrey Ritz (IntegrateIT.us)

	Modified:
	2018-08-28 jer - Modified the sequence of table creations and removed the circular reference from assigned_author and assigned_person, the reference set to be assigned_author -> assigned_person
*/	
...
(
  ...
  CONSTRAINT document_html_uuid_fkey FOREIGN KEY (allergy_html_uuid) REFERENCES "Mirth_Inbound".document_html (document_html_uuid) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT document_allergy_observation_uuid_fkey FOREIGN KEY (document_allergy_observation_uuid) REFERENCES "Mirth_Inbound".document_allergy_observation (document_allergy_observation_uuid) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
)
WITH (
  OIDS=FALSE
...
DROP TABLE IF EXISTS "Mirth_Inbound".document_header CASCADE;
